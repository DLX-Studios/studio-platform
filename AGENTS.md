...
- Keep `CARGO_TARGET_DIR` on a Unix filesystem (for example `$HOME/.cache/studio-cargo/...`), not on vfat/exfat checkouts, because Cargo must execute generated build scripts.
