...
fn registry() -> studio_plugin_registry::ExtensionRegistry {
    let trust =
        TrustStore::from_keys(pos_pack_trust_keys()).expect("fixture trust keys are valid");
...
fn full_walkthrough_admission_consent_install_lifecycle_removal_report() {
    ...
    registry
        .hooks_mut()
        .register(POS_PACK, LifecycleHook::Install, log_handler(&log, "install"));
    registry
        .hooks_mut()
        .register(
            POS_PACK,
    ...
    assert_eq!(log.lock().unwrap().as_slice(), ["install"]);
    ...
    registry.project_open(PROJECT, POS_PACK).expect("project open");
    assert_eq!(
        log.lock().unwrap().as_slice(),
        ["install", "activate", "project-open"]
    );
...
fn tampered_descriptor_is_rejected_before_activation() {
    ...
    assert_eq!(
        error.code(),
        RegistryErrorCode::AdmissionSignatureInvalid
    );
...
fn expired_compatibility_is_rejected_at_admission() {
    ...
    assert_eq!(
        error.code(),
        RegistryErrorCode::CompatibilityUnsupported
    );
}
...
fn third_party_descriptors_cannot_register_renderer_kinds_or_unknown_fields() {
    let hostile_kind = signed_variant(|descriptor| {
        descriptor.contributions.compositions.push(
    ...
    assert_eq!(
        error.code(),
        RegistryErrorCode::ContributionUnapproved
    );
...
fn contributions_deliver_compositions_settings_and_commands_as_data() {
    ...
    let field_kinds: Vec<&str> = groups[0]
        ...
        .filter_map(|field| match &field.kind {
            ...
            studio_plugin_registry::SettingsFieldType::DevicePicker { .. } => {
                Some("devicePicker")
            }
            _ => None,
        })
    ...
    assert!(actions
        .iter()
        .any(|action| action.id == commands[0].action));

    assert_eq!(descriptor.capabilities, vec![DeclaredCapability::PrinterSimulate]);
}
