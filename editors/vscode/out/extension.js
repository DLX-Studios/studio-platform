...
function startServer(context) {
    try {
        server = (0, node_child_process_2.spawn)(serverBinary(context), [], { stdio: ["pipe", "pipe", "inherit"] });
...
function degrade(reason) {
    if (!degraded) {
        degraded = true;
        void vscode.window.showWarningMessage(`Studio language features degraded (${reason}); highlighting and local commands still work.`);
    }
    server = null;
}
function sendRequest(method, params) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const onData = (chunk) => {
            ...
            const length = Number.parseInt(text.slice(0, boundary).replace("Content-Length:", "").trim(), 10);
            if (text.length < boundary + 4 + length) {
                return;
            }
            child.stdout?.off("data", onData);
            try {
                const message = JSON.parse(text.slice(boundary + 4, boundary + 4 + length));
...
function activeStudioDocument() {
    const document = vscode.window.activeTextEditor?.document;
    return document?.languageId === "studio" ? document : undefined;
}
function activate(context) {
    startServer(context);
    context.subscriptions.push(vscode.commands.registerCommand("studio.format", async () => {
        ...
        try {
            ...
            if (result.formatted !== undefined) {
                ...
                edit.replace(document.uri, new vscode.Range(0, 0, document.lineCount, 0), result.formatted);
                await vscode.workspace.applyEdit(edit);
            }
            else {
                void vscode.window.showWarningMessage(`Studio formatting unavailable: ${result.diagnostic ?? "unknown"}`);
            }
        }
        catch {
...
function runStudioCommand(args) {
    const terminal = vscode.window.terminals.find((candidate) => candidate.name === "studio") ??
...
async function newProjectFromGallery() {
    const templates = await readGalleryTemplates();
    if (templates === null) {
        void vscode.window.showWarningMessage("Studio templates unavailable: the `studio` binary is missing or has no gallery.");
        return;
    }
    const picked = await vscode.window.showQuickPick(templates.map((template) => ({
        label: template.id,
    ...
    const name = await vscode.window.showInputBox({
        ...
        validateInput: (value) => (0, templates_1.isValidProjectName)(value)
            ? undefined
    ...
    const terminal = vscode.window.terminals.find((candidate) => candidate.name === "studio") ??
        vscode.window.createTerminal("studio");
    terminal.show();
    terminal.sendText((0, templates_1.buildNewCommand)(folders[0].fsPath, name, picked.id));
    const open = "Open Folder";
    const choice = await vscode.window.showInformationMessage(`Scaffolding ${name} — reopen VS Code inside it to continue.`, open);
    if (choice === open) {
        await vscode.commands.executeCommand("vscode.openFolder", vscode.Uri.joinPath(folders[0], name));
    }
}
function readGalleryTemplates() {
    return new Promise((resolve) => {
        (0, node_child_process_1.execFile)(studioBinary(), ["new", "--list-templates", "--format", "json"], { timeout: 15_000 }, (error, stdout) => {
            if (error !== null) {
                resolve(null);
                return;
            }
            try {
                resolve((0, templates_1.parseTemplateList)(stdout));
            }
            catch {
                resolve(null);
            }
