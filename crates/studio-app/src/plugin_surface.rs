...
impl PluginSurface {
    ...
    /// Mount route of the committed tree, for reload route resolution.
    #[must_use]
    pub fn mount_route(&self) -> Option<String> {
        self.registry
            .snapshot(self.owner_id())
            .ok()
            .map(|snapshot| snapshot.route().to_owned())
    }
