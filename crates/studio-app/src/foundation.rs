...
pub struct FoundationGallery {
    ...
    last_reload_route: Option<crate::reload::ResolvedRoute>,
}
...
impl FoundationGallery {
    ...
    pub fn apply_reload_swap(&mut self, swap: crate::reload::PreparedSwap, cx: &mut Context<Self>) {
        // Record the decision before the move: shells that own navigation
        // apply the preserved/default route from here.
        self.last_reload_route = Some(swap.route.clone());
        crate::reload::apply_prepared_swap(&mut self.plugin_surface, swap);
        cx.notify();
    }

    /// Route decision of the most recently applied reload, if any.
    #[must_use]
    pub fn last_reload_route(&self) -> Option<&crate::reload::ResolvedRoute> {
        self.last_reload_route.as_ref()
    }
    ...
    fn deferred_native_element(
        ...
        div()
            .id(node.id.clone())
            .when(alert, |element| element.role(Role::Alert))
            .opacity(opacity)
