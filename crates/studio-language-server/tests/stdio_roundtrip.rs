...
fn frame(message: Value) -> Vec<u8> {
    ...
    format!("Content-Length: {}\r\n\r\n", payload.len()).into_bytes().into_iter().chain(payload).collect()
}
...
fn read_frames(mut bytes: &[u8]) -> Vec<Value> {
    ...
    while !bytes.is_empty() {
        ...
        let length: usize = header.strip_prefix("Content-Length: ").expect("content length").parse().expect("length");
...
fn drives_stdio_server_end_to_end_without_designer() {
    ...
    request.extend(frame(json!({"jsonrpc":"2.0","id":1,"method":"initialize","params":{}})));
    ...
    request.extend(frame(json!({"jsonrpc":"2.0","id":3,"method":"shutdown","params":null})));
    let mut output = Vec::new();
    server.serve(Cursor::new(request), &mut output).expect("stdio session succeeds");
    let responses = read_frames(&output);
    assert!(responses.iter().any(|value| value.get("id") == Some(&json!(1))));
    assert!(responses.iter().any(|value| value.get("id") == Some(&json!(2))));
    assert!(responses.iter().any(|value| value.get("method") == Some(&json!("textDocument/publishDiagnostics"))));
}
