...
/// Whether a source uses Svelte-shaped Studio Script (anything without the
/// legacy `studio 1` version header).
#[must_use]
pub fn is_svelte_source(source: &str) -> bool {
    !source.trim_start().starts_with("studio 1")
}

/// Filename for upstream tooling from a document URI.
fn filename_for_uri(uri: &str) -> String {
    uri.rsplit('/').next().unwrap_or(uri).to_owned()
}

/// Component name guess from a document URI (`Card.studio` → `Card`).
fn component_name_for_uri(uri: &str) -> String {
    let filename = filename_for_uri(uri);
    filename
        .strip_suffix(".studio")
        .or_else(|| filename.strip_suffix(".svelte"))
        .unwrap_or("component")
        .to_owned()
}
...
impl LanguageServer {
    ...
    pub fn diagnostics(&self, uri: &str) -> Vec<Diagnostic> {
        ...
        if is_svelte_source(source) {
            return lint::merged_diagnostics(
                source,
                &filename_for_uri(uri),
                &component_name_for_uri(uri),
            );
        }
    ...
    pub fn completion(&self, uri: &str, position: Position) -> CompletionList {
        ...
        if is_svelte_source(source) {
            return self.svelte_completion(source, position);
        }
    ...
    pub fn hover(&self, uri: &str, position: Position) -> Option<Hover> {
        ...
        if is_svelte_source(source) {
            return self.svelte_hover(source, position);
        }
        let offset = offset_for_position(source, position);
    ...
    /// Format one open document, returning the formatted text or a safe
    /// message. Legacy sources use the canonical printer; Svelte-shaped
    /// sources use the composed formatter (idempotent by contract).
    #[must_use]
    pub fn format_document(&self, uri: &str) -> Result<String, String> {
        let Some(source) = self.source(uri) else {
            return Err("unknown document".to_owned());
        };
        if !is_svelte_source(source) {
            return Err("legacy sources format through `studio fmt`".to_owned());
        }
        let path = std::path::PathBuf::from(format!("/tmp/{}.svelte", component_name_for_uri(uri)));
        format::format_studio(source, &path)
    }

    /// Rename the binding under a position across its file. Returns every
    /// location the client must rewrite (the client applies `new_name`).
    #[must_use]
    pub fn rename(&self, uri: &str, position: Position, new_name: &str) -> Vec<Location> {
        if !is_valid_rename(new_name) {
            return Vec::new();
        }
        self.binding_locations(uri, position)
    }

    /// Reference locations for the binding under a position.
    #[must_use]
    pub fn references(&self, uri: &str, position: Position) -> Vec<Location> {
        self.binding_locations(uri, position)
    }

    /// Shared definition-plus-references lookup for Svelte-shaped sources.
    fn binding_locations(&self, uri: &str, position: Position) -> Vec<Location> {
        let Some(source) = self.source(uri) else {
            return Vec::new();
        };
        if !is_svelte_source(source) {
        ...
        let engine = studio_script::rsvelte_adapter::AdapterEngine::new();
        let Ok(parsed) = engine.parse_module(source, &filename_for_uri(uri)) else {
            return Vec::new();
        };
        let Ok(script) = studio_script::validate::validate_module(source, &parsed) else {
            return Vec::new();
        };
        let symbols = projection::index_module(&parsed, &script);
        let offset = position_to_offset(source, position);
        let Some(symbol) = projection::definition_of(&symbols, source, offset) else {
            return Vec::new();
        };
        projection::references_of(&symbols, source, &symbol.name)
            .into_iter()
            .map(|(start, end)| Location {
                uri: uri.to_owned(),
                range: crate::Range {
                    start: position_for_offset(source, start as usize),
                    end: position_for_offset(source, end as usize),
                },
            })
            .collect()
    }

    /// Go-to-definition for Svelte-shaped sources: workspace symbols first,
    /// then the binding declaration.
    fn svelte_definition(&self, uri: &str, source: &str, position: Position) -> Vec<Location> {
        let offset = position_to_offset(source, position);
        if let Some((symbol, _)) = symbol_at(source, offset as usize) {
            if let Some(field) = symbol.strip_prefix("$item.") {
                return self
                    .workspace
                    .schemas
                    .values()
                    .filter_map(|schema| {
                        schema.fields.get(field).map(|field| field.location.clone())
                    })
                    .collect();
            }
        ...
        let symbols = projection::index_module(&parsed, &script);
        let offset = position_to_offset(source, position);
        projection::definition_of(&symbols, source, offset)
            .into_iter()
            .map(|symbol| Location {
                uri: uri.to_owned(),
                range: crate::Range {
                    start: position_for_offset(source, symbol.span.0 as usize),
                    end: position_for_offset(source, symbol.span.1 as usize),
                },
            })
            .collect()
    }
    ...
    pub fn definition(&self, uri: &str, position: Position) -> Vec<Location> {
        ...
        if is_svelte_source(source) {
            return self.svelte_definition(uri, source, position);
        }
        let offset = offset_for_position(source, position);
    ...
    /// Svelte-shaped completion: catalog components in tag position, runes on
    /// `$`, nothing elsewhere (deterministic, closed sets only).
    fn svelte_completion(&self, source: &str, position: Position) -> CompletionList {
        let offset = offset_for_position(source, position);
        let before = &source[..offset.min(source.len())];
        let mut items = BTreeMap::<String, CompletionItem>::new();
        let tag_context = before.rfind('<').is_some_and(|start| {
            start > before.rfind('>').unwrap_or(0) && !before[start..].contains('>')
        });
        // Inside a `{...}` attribute expression the cursor is in code, not in
        // tag syntax: offer expression completions instead of tag names.
        let in_expression = before
            .rfind('{')
            .is_some_and(|open| open > before.rfind('}').unwrap_or(0));
        if tag_context && !in_expression {
            for (name, documentation) in intelligence::component_completions() {
                insert_completion(
                    &mut items,
                    &name,
                    "Studio component",
            ...
            for definition in self.workspace.components.values() {
                insert_completion(
                    &mut items,
                    &definition.name,
                    &definition.detail,
        ...
        if before.ends_with('$')
            || before.rfind('$').is_some_and(|start| {
                !before[start..]
                    ...
                    && before[start..].chars().all(|character| {
                        character.is_ascii_alphanumeric() || character == '_' || character == '$'
                    })
            })
        {
            for (name, documentation) in intelligence::RUNES {
                insert_completion(
                    &mut items,
                    name,
                    "Studio rune",
        ...
        if in_expression {
            let prefix = completion_prefix(source, offset);
            if prefix.starts_with("token.") || prefix.starts_with("$token.") {
                for definition in self.workspace.tokens.values() {
                    insert_completion(
                        &mut items,
                        &definition.name,
            ...
            if prefix.starts_with("plugin.") || prefix.starts_with("sdk.") {
                for definition in self.workspace.plugins.values() {
                    insert_completion(
                        &mut items,
                        &definition.name,
    ...
    /// Svelte-shaped hover: workspace symbols first, then catalog
    /// documentation for component tags.
    fn svelte_hover(&self, source: &str, position: Position) -> Option<Hover> {
        let offset = offset_for_position(source, position);
        if let Some((symbol, range)) = symbol_at(source, offset) {
            if let Some(field) = symbol.strip_prefix("$item.") {
                for schema in self.workspace.schemas.values() {
                    if let Some(field) = schema.fields.get(field) {
                        return Some(hover_for(
        ...
        let (name, _) = projection::identifier_at(source, offset as u32)?;
        let documentation = intelligence::component_documentation(&name)?;
        let (start, end) = projection::identifier_at(source, offset as u32)
            .map(|(_, span)| span)
            .unwrap_or((0, 0));
        Some(hover_for(
            &name,
    ...
    fn handle_message(&mut self, message: Value) -> (Option<Value>, Vec<Value>) {
        ...
        let result = match method {
            ...
            "textDocument/formatting" => {
                let uri = params.pointer("/textDocument/uri").and_then(Value::as_str).unwrap_or_default();
                match self.format_document(uri) {
                    Ok(formatted) => json!({"formatted": formatted}),
                    Err(message) => json!({"diagnostic": message}),
                }
            }
            "textDocument/rename" => {
                let (uri, position) = match request_document_position(&params) {
                ...
                let new_name = params.get("newName").and_then(Value::as_str).unwrap_or_default();
                let edits: Vec<Value> = self
                    .rename(&uri, position, new_name)
                    .into_iter()
                    .map(|location| serde_json::to_value(location).unwrap_or(Value::Null))
                    .collect();
                json!({"edits": edits})
            }
...
/// Byte offset for an LSP position, clamped into the source.
fn position_to_offset(source: &str, position: Position) -> u32 {
    offset_for_position(source, position) as u32
}

/// Rename targets must be plain identifiers.
