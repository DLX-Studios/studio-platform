...
impl NativeProductBootstrap {
    ...
    /// Create, authenticate, and enter the dashboard for a new local identity.
    ///
    /// This is the first-launch form path. Keeping both operations in the
    /// host-owned bootstrap prevents the native shell from manufacturing a
    /// session or briefly exposing an authenticated route without one.
    pub fn create_identity_and_sign_in_blocking(
        &mut self,
        display_name: impl Into<String>,
        password: impl Into<Vec<u8>>,
        confirmation: impl Into<Vec<u8>>,
        ...
        let password = password.into();
        let summary =
            self.create_identity_blocking(display_name, password.clone(), confirmation)?;
        self.sign_in_blocking(&summary.identity_id, &password, remember)?;
        Ok(summary)
    }
...
impl NativeProductShell {
    ...
    fn submit_create(&mut self, cx: &mut Context<Self>) {
        ...
        if name.trim().is_empty() || password != confirmation {
            self.identity_error =
                Some("Display name and matching passwords are required.".to_owned());
            cx.notify();
            return;
        }
        let service = Arc::clone(&self.bootstrap.identity_service);
        ...
        let task = cx.background_executor().spawn(async move {
            ...
            let summary = block_on_native(service.create_identity(request))?;
            let session =
                block_on_native(service.sign_in(&summary.identity_id, &password, remember))?;
            let snapshot = block_on_native(service.snapshot())?;
            Ok::<_, IdentityError>((summary, session, snapshot))
        });
        cx.spawn(async move |this, cx| {
            let result = task.await;
            this.update(cx, |shell, cx| {
                shell.identity_loading = false;
                match result {
                    Ok((_, session, snapshot)) => {
                        shell.bootstrap.state.identity.refresh(snapshot);
                        shell.bootstrap.install_session(session);
                        shell.input_route = None;
                        shell.identity_error = None;
                    }
                    Err(error) => {
                        shell.identity_error = Some(safe_identity_error(&error));
                    }
                }
                cx.notify();
            })
    ...
    fn submit_gate(&mut self, identity_id: String, unlock: bool, cx: &mut Context<Self>) {
        ...
        let service = Arc::clone(&self.bootstrap.identity_service);
        let remember = self.remember_session;
        let worker_identity_id = identity_id.clone();
        self.identity_loading = true;
        self.identity_error = None;
        cx.notify();
        let task = cx.background_executor().spawn(async move {
            let session = if unlock {
                block_on_native(service.unlock(&worker_identity_id, &password, remember))?
            } else {
                block_on_native(service.sign_in(&worker_identity_id, &password, remember))?
            };
            let snapshot = block_on_native(service.snapshot())?;
            Ok::<_, IdentityError>((session, snapshot))
        });
        cx.spawn(async move |this, cx| {
            let result = task.await;
            this.update(cx, |shell, cx| {
                ...
                match result {
                    Ok((session, snapshot)) => {
                        ...
                        shell.bootstrap.install_session(session);
                        shell.input_route = None;
                        shell.dashboard = None;
                        shell.dashboard_error = None;
                    }
                    Err(error) => {
                        if !unlock && error.code() == studio_host::IdentityErrorCode::WrongPassword
                        {
                            shell.bootstrap.state_mut().navigate(ProductRoute::Unlock {
...
mod tests {
    ...
    #[test]
    fn new_project_opens_with_visible_starter_canvas() {
        ...
        let project_id = ProjectId::new("smoke-project").expect("project identity is valid");
        let actor = Actor {
            id: ActorId::new("designer-smoke").expect("actor identity is valid"),
            kind: ActorKind::Human,
            display_name: "Designer smoke test".to_owned(),
        };

        let (model, _) = block_on_native(open_or_create_focus(store, project_id, actor))
            .expect("new project opens");
        let snapshot = model.snapshot();
        let canvas = snapshot
            .canvas
            .expect("new project has a canvas projection");
        assert_eq!(canvas.id.as_str(), "canvas");
        assert_eq!(canvas.children.len(), 1);
        assert_eq!(canvas.children[0].id.as_str(), "headline");
        assert_eq!(
            canvas.children[0].props["text"],
            serde_json::Value::String("Welcome to Studio Designer".to_owned())
        );
        assert!(include_str!("focus_view.rs").contains("focus-canvas-surface"));
    }
}
                ...
                content = content
                    .child(
                        Button::new("submit-create-identity")
                            .label(if self.identity_loading {
                                "Creating identity…"
                            } else {
                                "Create identity"
                            })
                            .disabled(self.identity_loading)
                            .primary()
