...
#[test]
fn fresh_directory_identity_creation_enters_the_dashboard() {
    let directory = tempfile::tempdir().expect("temporary Studio data directory");
    let mut bootstrap = NativeProductBootstrap::open(directory.path()).expect("bootstrap opens");
    bootstrap
        .dismiss_welcome()
    ...
    bootstrap
        ...
        .expect("identity creation authenticates the new identity");

    assert!(bootstrap.state().is_authenticated());
    assert_eq!(bootstrap.state().route(), &ProductRoute::Dashboard);
    assert_eq!(bootstrap.state().identity().identities().len(), 1);
    assert_eq!(bootstrap.state().identity().sessions().len(), 1);
}
