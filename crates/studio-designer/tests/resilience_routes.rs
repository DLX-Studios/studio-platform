...
fn project_settings_center_preserves_project_identity_in_route() {
    let route =
        ResilienceRoute::from_project_settings("restaurant-local", ResilienceCenter::Recovery)
            .unwrap();
...
fn project_settings_route_rejects_delimiters_and_controls() {
    for project_id in [
        ...
        assert!(
            ResilienceRoute::from_project_settings(project_id, ResilienceCenter::Conflicts)
                .is_err()
        );
    }
}
