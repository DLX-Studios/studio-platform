...
#[test]
fn accepts_optional_assemblyscript_abort_import() {
    // asc always emits `env.abort`; the host links it as a terminal trap.
    let with_abort = abi_builder().import_function(
        "env",
        "abort",
        signature(
            &[
    ...
    validate(with_abort).unwrap();

    // Wrong abort signature is still rejected.
    let bad_abort =
        abi_builder().import_function("env", "abort", signature(&[ValueType::I32], &[]));
    assert_eq!(
        validate(bad_abort).unwrap_err(),
        PolicyErrorCode::ImportInvalid
    );

    // Abort alone without `studio_host.emit` is still rejected.
    let abort_only = WasmFixtureBuilder::new()
        .import_function(
            "env",
            "abort",
            signature(
        ...
        .table(TableSpec::new(0).maximum(1024));
    assert_eq!(
        validate(abort_only).unwrap_err(),
        PolicyErrorCode::ImportInvalid
    );
}
