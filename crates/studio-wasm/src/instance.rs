...
impl PluginInstance {
    ...
    pub fn instantiate(
        ...
        // AssemblyScript's abort import: record the abort site and return.
        // The invoke path converts the record into a terminal guest-trapped
        // failure; asc emits `unreachable` after abort sites, so a natively
        // trapping continuation is preserved when present. Coordinates only —
        // no guest-memory reads on the abort path.
        linker
            .func_wrap(
                "env",
                "abort",
                |mut caller: Caller<'_, StoreData>,
                 ...
                 line: i32,
                 column: i32| {
                    caller.data_mut().aborted =
                        Some(format!("guest aborted at line {line}, column {column}"));
                },
            )
            .map_err(|error| RuntimeError::Instantiation(error.to_string()))?;
        let instance = linker
    ...
    fn invoke_with_fuel(
        ...
        if let Some(site) = self.store.data_mut().aborted.take() {
            return Err(self.terminate(RuntimeError::GuestTrapped(site)));
        }
