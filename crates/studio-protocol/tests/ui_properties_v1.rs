...
#[test]
fn child_cardinality_matches_mount_rules() {
    use studio_protocol::{ChildCardinality, child_cardinality};
    // One representative per contract family.
    assert_eq!(child_cardinality(NodeKind::Column), ChildCardinality::Any);
    assert_eq!(child_cardinality(NodeKind::Grid), ChildCardinality::Any);
    assert_eq!(
        child_cardinality(NodeKind::Card),
        ChildCardinality::AtMostOne
    );
    assert_eq!(
        child_cardinality(NodeKind::ScrollView),
        ChildCardinality::AtMostOne
    );
    assert_eq!(child_cardinality(NodeKind::Text), ChildCardinality::None);
    assert_eq!(child_cardinality(NodeKind::Button), ChildCardinality::None);
    assert_eq!(child_cardinality(NodeKind::Badge), ChildCardinality::None);
    assert_eq!(
        child_cardinality(NodeKind::Tooltip),
        ChildCardinality::ExactlyOne
    );
}

#[test]
fn known_property_mirror_agrees_with_mount_validation() {
    use studio_protocol::is_known_property;
    let limits = ProtocolLimits::default();
    // Representative corpus across prop families: layout numbers, closed
    // vocabularies, events, assets, transitions, and option lists.
    let known = [
        (NodeKind::Box, "padding"),
        (NodeKind::Box, "background"),
        (NodeKind::Column, "gap"),
        (NodeKind::Grid, "columns"),
        (NodeKind::ScrollView, "axis"),
        (NodeKind::Text, "text"),
        (NodeKind::Text, "typography_role"),
        (NodeKind::Button, "label"),
        (NodeKind::Button, "variant"),
        (NodeKind::Button, "on_pressed"),
        (NodeKind::Badge, "label"),
        (NodeKind::Tag, "variant"),
        (NodeKind::Icon, "name"),
        (NodeKind::Image, "asset"),
        (NodeKind::Avatar, "fallback"),
        (NodeKind::Select, "options"),
        (NodeKind::OtpInput, "length"),
        (NodeKind::SecretInput, "on_ready"),
        (NodeKind::Dialog, "open"),
        (NodeKind::CommandPalette, "commands"),
        (NodeKind::Card, "padding"),
    ];
    for (kind, property) in known {
        assert!(
            is_known_property(kind, property, limits),
            "{property} is known for {kind:?}"
        );
    }
    let unknown = [
        (NodeKind::Card, "title"),
        (NodeKind::Card, "label"),
        (NodeKind::Text, "label"),
        (NodeKind::Button, "columns"),
        (NodeKind::Grid, "label"),
        (NodeKind::Badge, "padding"),
        (NodeKind::Box, "bogus"),
    ];
    for (kind, property) in unknown {
        assert!(
            !is_known_property(kind, property, limits),
            "{property} is unknown for {kind:?}"
        );
    }
}
