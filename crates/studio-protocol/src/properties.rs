...
/// Child-count contract of one component kind, shared by mount-time
/// validation and check-time diagnostics so the two can never drift.
...
pub enum ChildCardinality {
    ...
    /// Zero or one child.
    AtMostOne,
    /// No children: leaf content comes from props.
    None,
    /// Exactly one child.
    ExactlyOne,
}

/// Resolve a kind's child contract by probing the mount-time validator.
///
/// Probing (rather than a parallel match) keeps one rule source: any edit
/// to `validate_child_count` flows through automatically.
#[must_use]
pub fn child_cardinality(kind: NodeKind) -> ChildCardinality {
    fn allows(kind: NodeKind, count: usize) -> bool {
        let leaf = || UiNode {
            id: "probe".to_owned(),
            kind: NodeKind::Empty,
            props: BTreeMap::new(),
        ...
        let node = UiNode {
            ...
            kind,
            props: BTreeMap::new(),
            children: (0..count).map(|_| leaf()).collect(),
        };
        validate_child_count(&node).is_ok()
    }
    match (allows(kind, 0), allows(kind, 1), allows(kind, 2)) {
        (true, true, true) => ChildCardinality::Any,
        (true, true, false) => ChildCardinality::AtMostOne,
        (true, false, false) => ChildCardinality::None,
        // Every group admits a prefix pattern over 0..=2, so only the
        // exactly-one shape is left.
        _ => ChildCardinality::ExactlyOne,
    }
}

/// Whether a property name is known for a kind, regardless of value.
///
/// Probes the mount-time validator with one dummy per value shape, so
/// renames and additions flow through automatically. Closed-vocabulary
/// props need a valid word each: the dummy strings cover every
/// `matches_string` vocabulary in this file, so adding a new vocabulary
/// requires extending this set (guarded by the corpus test).
/// Check-time diagnostics use this; value checking stays a mount-time
/// concern.
#[must_use]
pub fn is_known_property(kind: NodeKind, property: &str, limits: ProtocolLimits) -> bool {
    // Every `matches_string` vocabulary below, plus the open shapes.
    const WORDS: &[&str] = &[
        "x",
        "body",
        "label",
    ...
    for word in WORDS {
        if validate_specific(kind, property, &Value::String((*word).to_owned()), limits)
            .is_ok_and(|known| known)
        {
            return true;
        }
    }
    // Integer- and float-shaped numbers: integer-only validators
    // (`columns`, counts) need the integer forms.
    for number in [
        serde_json::json!(0),
        serde_json::json!(1),
        serde_json::json!(0.5),
    ] {
        if validate_specific(kind, property, &number, limits).is_ok_and(|known| known) {
            return true;
        }
    }
    if validate_specific(kind, property, &Value::Bool(true), limits).is_ok_and(|known| known) {
        return true;
    }
    if validate_specific(
        kind,
        property,
    ...
    validate_specific(
        kind,
        property,
        &serde_json::json!({"curve": "linear", "duration_ms": 100}),
        limits,
    )
    .is_ok_and(|known| known)
}
