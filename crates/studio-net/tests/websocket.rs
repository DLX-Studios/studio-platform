...
struct ScriptedTransport {
    plans: Mutex<VecDeque<Result<VecDeque<Result<Option<Vec<u8>>, WebSocketTransportError>>, WebSocketTransportError>>>,
}

impl ScriptedTransport {
    fn new(
        plans: Vec<Result<VecDeque<Result<Option<Vec<u8>>, WebSocketTransportError>>, WebSocketTransportError>>,
    ) -> Self {
        Self {
...
impl WebSocketTransport for ScriptedTransport {
    fn connect(
        ...
        self.plans
            ...
            .map(|receives| Box::new(ScriptedConnection { receives }) as Box<dyn WebSocketConnection>)
    }
}
...
fn broker(
    ...
    let mut broker = WebSocketBroker::new(transport, WebSocketBrokerLimits {
...
fn admission_and_typed_lifecycle_are_host_owned() {
    ...
    assert!(matches!(session.next_event(), Some(WebSocketEvent::Opened { .. })));
    assert!(matches!(session.next_event(), Some(WebSocketEvent::Message(Value::Object(_)))));
...
fn outbound_schema_and_subprotocol_admission_fail_before_connect() {
    ...
    let error = broker
        ...
        .unwrap_err();
    assert_eq!(error.code(), BrokerErrorCode::WebSocketSubprotocolNotAllowed);
}
...
fn reconnect_is_scheduled_by_host_and_invalid_peer_data_is_not_delivered() {
    ...
    let session = broker
        ...
        .unwrap();
    assert!(matches!(session.next_event(), Some(WebSocketEvent::Reconnecting { attempt: 1, .. })));
    assert!(matches!(session.next_event(), Some(WebSocketEvent::Opened { .. })));
    assert!(matches!(session.next_event(), Some(WebSocketEvent::Message(_))));
    assert!(matches!(session.next_event(), Some(WebSocketEvent::Closed)));
}
