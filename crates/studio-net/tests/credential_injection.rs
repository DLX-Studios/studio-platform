...
use common::{broker, code_of, named_secret_group, secret_fixture, ORIGIN};
...
fn named_secret_is_injected_at_send_time_with_prefix() {
    ...
    assert!(
        request.headers.iter().any(|(name, value)| name == "authorization"
...
fn credential_echoing_response_is_rejected_before_guest_visibility() {
    ...
    fixture
        .transport
        .respond(200, "text/plain", &format!(r#"{{"echo":"{SECRET_VALUE}"}}"#));
    let error = fixture
        .broker
        .execute(BrokerRequest::new(ORIGIN, HttpMethod::Get, "/v1/private"))
...
fn missing_configured_value_fails_closed_with_safe_code() {
    ...
    let declared_key =
        ProtectedSecretKey::new("payments.key", "REST credential for route group using payments.key")
            .expect("key");
    let empty_partition = secret_fixture("unused.name", "unused-value");
    let handle = empty_partition.injection_handle(&[declared_key.clone(), empty_partition.key.clone()]);
...
fn revoked_named_secret_fails_closed() {
    let fixture = broker(&[named_secret_group("authorization", None)]);
    let secret = secret_fixture("payments.key", SECRET_VALUE);
    let principal_partition = secret.store;
    ...
    .expect("principal");
    let application = principal_partition
        .for_application(&principal, ApplicationEnvironment::Development)
        .expect("partition");
    application.revoke(&secret.key).expect("revoke");
    let handle = principal_partition.broker_injection_handle([secret.key.clone()]);
    fixture.broker.set_named_secret_injector(Arc::new(handle));
    let error = fixture
        .broker
        .execute(BrokerRequest::new(ORIGIN, HttpMethod::Get, "/v1/private"))
