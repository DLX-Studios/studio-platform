...
use common::{
    broker, broker_with_limits, chunk_values, code_of, drain_stream, sse_group, ReadStep,
};
...
fn chunks_are_framed_across_reads_and_validated() {
    ...
    let events = drain_stream(&fixture.broker.open_stream(stream_request()).expect("stream"));
    assert!(events.first().is_some_and(|event| matches!(event, StreamEvent::Opened)));
    assert_eq!(
        chunk_values(&events),
        vec![
...
fn guest_cancellation_stops_delivery() {
    ...
    for index in 0..1000 {
        ...
        steps.push(ReadStep::Bytes(format!("data: {{\"text\":\"{index}\"}}\n\n").into_bytes()));
    }
    fixture.transport.plan_stream(steps);
    let handle = fixture.broker.open_stream(stream_request()).expect("stream");
...
fn transport_failure_triggers_host_owned_reconnect_with_last_event_id() {
    ...
    assert!(events.iter().any(|event| matches!(event,
        StreamEvent::RetryScheduled { attempt: 1, .. })));
