...
pub mod ir;
pub mod lower;
pub mod lower_studio;
pub mod rsvelte_adapter;
pub mod types;
pub mod validate;
...
/// Compile one `.studio` source into a typed v2 [`ir::StudioModule`].
///
/// The pipeline is adapter (parse) → validator (portable subset) → lowerer
/// (typed IR). Any stage can fail with spanned diagnostics; no partial
/// module is ever returned.
///
/// # Errors
///
/// Returns every diagnostic from the failing stage.
pub fn compile_studio(
    source: &str,
    filename: &str,
    name: &str,
) -> Result<ir::StudioModule, Vec<Diagnostic>> {
    let engine = rsvelte_adapter::AdapterEngine::new();
    let parsed = engine.parse_module(source, filename)?;
    let script = validate::validate_module(source, &parsed)?;
    lower_studio::lower_studio(source, &parsed, &script, name)
}
...
/// Stable diagnostic code for a construct outside the portable subset
/// (browser APIs, eval, dynamic imports, async, reflection, host calls).
pub const CODE_SUBSET_BOUNDARY: &str = "STUDIO300";
/// Stable diagnostic code for a non-serializable or explicitly untyped value.
pub const CODE_NON_SERIALIZABLE: &str = "STUDIO301";
/// Stable diagnostic code for an unsupported rune or lifecycle API.
pub const CODE_UNSUPPORTED_RUNE: &str = "STUDIO302";
/// Stable diagnostic code for an untyped or uninferrable binding.
pub const CODE_UNINFERRED_TYPE: &str = "STUDIO310";
/// Stable diagnostic code for an expression without a lowering rule.
pub const CODE_NO_LOWERING_RULE: &str = "STUDIO320";
/// Stable diagnostic code for a call to an unknown or unapproved function.
pub const CODE_UNKNOWN_CALL: &str = "STUDIO321";
/// Stable diagnostic code for an unknown component, prop, event, route, or
/// asset reference in Studio Script sources.
pub const CODE_STUDIO_UNKNOWN: &str = "STUDIO330";
/// Stable diagnostic code for a node shape the catalog cannot mount:
/// unknown props, element children under leaves, or over-full
/// single-child containers.
pub const CODE_SHAPE: &str = "STUDIO015";
/// Stable diagnostic code for module graph problems (unresolvable imports,
/// cycles, invalidation failures).
pub const CODE_MODULE_GRAPH: &str = "STUDIO340";
