...
fn prerequisite_evidence() -> Vec<PrerequisiteEvidence> {
    [
        (
            24,
            "integrated",
            "studio-host::rbac enforces employee row scope (RowScope::own/record_ids/field_equals) \
             with collection grants, credential throttling, and host-only digests; covered by \
             crates/studio-host/tests/rbac.rs",
        ),
        (
            25,
            "integrated",
            "studio-host::topology validates center/station/operation identities, pairing tokens, \
             and conflicts; covered by crates/studio-host/tests/topology.rs and center_protocol.rs",
        ),
        (
            27,
            "integrated",
            "studio-host::workflows provides the workflow scheduler seam with triggers, retry, and \
             missed-fire policies; covered by crates/studio-host/tests/workflows.rs",
        ),
        (
            29,
            "integrated",
            "studio-host::audit_log provides the host-owned hash-chained application audit seam; \
             covered by crates/studio-host/tests/audit_log.rs",
        ),
        (
            30,
            "integrated",
            "studio-update-channel admits Ed25519-signed update documents through the \
             studio_package trust store with artifact digests; covered by \
             crates/studio-update-channel/tests/update_channel.rs",
        ),
        (
            33,
            "integrated",
        ...
        (
            34,
            "not_integrated",
            "generic POS path certification requires the physical three-station baseline run; \
             explicitly deferred to the release hardware gate",
        ),
        (
            49,
            "integrated",
            "studio-designer::content_editors drives collection schema and typed-form validation \
             through DesignerSession; covered by crates/studio-designer/tests/sprint04_surfaces.rs",
        ),
        (
            51,
            "integrated",
            "studio-designer::conversation_composer wraps the studio-design agent conversation \
             state with typed submit/receive flow; covered by \
             crates/studio-designer/tests/sprint04_surfaces.rs",
        ),
        (
            53,
            "integrated",
            "studio-designer::plugin_template_ux applies tracked plugin installs and generated \
             settings edits through DesignerSession; covered by \
             crates/studio-designer/tests/sprint04_surfaces.rs",
        ),
        (
            55,
            "integrated",
            "project dashboard is present in the current main ancestry",
        ),
    ]
    .into_iter()
...
fn verification_gaps() -> Vec<VerificationGap> {
    ...
    let outstanding: Vec<u8> = prerequisite_evidence()
        .into_iter()
        .filter(|prerequisite| prerequisite.status == "not_integrated")
        .map(|prerequisite| prerequisite.ticket)
        .collect();
    if outstanding.is_empty() {
        gaps.push(VerificationGap {
            gate: "prerequisites".to_owned(),
            reason: "Every prerequisite seam is integrated in the workspace.".to_owned(),
            blocking: false,
        });
    } else {
        let tickets = outstanding
            .iter()
            .map(|ticket| ticket.to_string())
            .collect::<Vec<_>>()
            .join(", ");
        gaps.push(VerificationGap { gate: "prerequisites".to_owned(), reason: format!("Prerequisites still marked not_integrated must land or stay explicitly scoped before this harness can certify production seams: tickets {tickets}."), blocking: true });
    }
    gaps
}
