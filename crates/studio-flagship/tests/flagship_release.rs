...
fn demo_day_report_passes_deterministic_gates_but_keeps_external_blockers_visible() {
    ...
    // Host seams landed in package 01/04 are now reported as integrated with
    // evidence-backed reasons instead of stale static claims.
    for ticket in [24, 25, 27, 29, 30, 49, 51, 53] {
        assert!(
            report
                .prerequisites
                .iter()
                .any(|item| item.ticket == ticket && item.status == "integrated"),
            "ticket {ticket} must be integrated"
        );
    }
    // Generic POS path certification stays explicitly scoped to the release
    // hardware gate, so the prerequisites gap remains visible and blocking.
    assert!(
        report
            .prerequisites
            .iter()
            .any(|item| item.ticket == 34 && item.status == "not_integrated")
    );
    let prerequisites_gap = report
        .verification_gaps
        .iter()
        .find(|gap| gap.gate == "prerequisites")
        .expect("prerequisites gap is always reported");
    assert!(prerequisites_gap.blocking);
    assert!(prerequisites_gap.reason.contains("34"));
}
