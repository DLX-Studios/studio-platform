...
impl InstallationHost for Host {
    fn install(&mut self, id: &str, update: &VerifiedUpdate) -> Result<(), HostError> {
        self.installs.push(format!("{id}:{}", update.document().version));
        Ok(())
    }

    fn health_check(&mut self, _id: &str, _version: &str) -> Result<(), HostError> {
        if self.fail_health { Err(HostError::Health) } else { Ok(()) }
    }
...
fn signed_candidate_is_admitted_and_tampering_is_rejected() {
    ...
    let signed = SignedUpdate {
        signature: ed25519_dalek::Signer::sign(&signing, &canonical_document_bytes(&serde_json::to_value(&document).unwrap()).unwrap()).to_bytes().to_vec(),
        document,
        artifact,
    };
...
fn rollout_activates_and_health_failure_rolls_back() {
    ...
    let signing = ed25519_dalek::SigningKey::from_bytes(&[7; 32]);
    let trust = TrustStore::from_keys([TrustedPublisherKey {
        publisher_id: "publisher".to_owned(), key_id: "key".to_owned(),
        verifying_key: signing.verifying_key().to_bytes(), enabled: true,
    }]).unwrap();
    let value = serde_json::to_value(&document).unwrap();
    let signature = ed25519_dalek::Signer::sign(&signing, &canonical_document_bytes(&value).unwrap()).to_bytes().to_vec();
    let update = VerifiedUpdate::admit(SignedUpdate { document, signature, artifact }, &trust).unwrap();
    let mut store = MemoryStateStore::default();
    baseline(&mut store, "device-a");
    let mut channel = UpdateChannel::new(store, "stable").unwrap();
    let mut host = Host { fail_health: false, installs: Vec::new(), rollbacks: Vec::new() };
    let report = channel.roll_out(&update, &["device-a".to_owned()], &mut host).unwrap();
    assert_eq!(report.outcomes, vec![InstallationOutcome::Activated]);
    assert_eq!(channel.state_store().states()["device-a"].active_version, "2.0.0");
    ...
    let next_signature = signing.sign(&canonical_document_bytes(&next_value).unwrap()).to_bytes().to_vec();
    let next = VerifiedUpdate::admit(SignedUpdate { document: next_document, signature: next_signature, artifact: b"update".to_vec() }, &trust).unwrap();
    let mut failing = UpdateChannel::new(channel.state_store().clone(), "stable").unwrap();
    host.fail_health = true;
    let report = failing.roll_out(&next, &["device-a".to_owned()], &mut host).unwrap();
    assert_eq!(report.outcomes, vec![InstallationOutcome::Failed(studio_update_channel::UpdateErrorCode::HealthCheck)]);
    assert_eq!(failing.state_store().states()["device-a"].active_version, "2.0.0");
    assert!(failing.state_store().states()["device-a"].history.iter().any(|event| event.kind == InstallationEventKind::RolledBack));
}
