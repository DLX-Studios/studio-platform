import { n as noop } from "./chunks/functions.js";
import { F as handle_error_and_jsonify, G as stringify_remote_arg, Ot as MUTATIVE_METHODS, Q as stringify, St as app_dir, U as create_remote_key, W as parse_remote_arg, Z as parse, at as normalize_issue, it as flatten_issues, nt as create_field_proxy, ot as parse_form_key, r as prerendering, rt as deep_set, st as set_nested_value } from "./chunks/server.js";
import { error } from "@sveltejs/kit";
import { HandledHttpError, HttpError, ValidationError } from "@sveltejs/kit/internal";
			...
			fn: async (data, meta, form_data) => {
				...
				if (validated?.issues !== void 0) handle_issues(output, validated.issues, form_data, __.id);
					...
					try {
						...
						if (e instanceof ValidationError) handle_issues(output, e.issues, form_data, __.id);
						else throw e;
					}
				}
...
function handle_issues(output, issues, form_data, form_id) {
	...
	if (form_data) {
		...
		for (const field_name of form_data.keys()) {
			...
			set_nested_value(output.input, field, field.is_array ? values : values[0]);
		}
	}
}
/**
...
function requested(query, limit) {
	...
	*/
	...
	for (const payload of skipped) record_failure(payload, new HttpError({
		status: 400,
