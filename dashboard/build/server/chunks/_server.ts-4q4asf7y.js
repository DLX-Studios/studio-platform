...
var POST = async ({ request, cookies }) => {
  ...
  if (body.openrouterKey)
    try {
      if (!(await fetch("https://openrouter.ai/api/v1/key", { headers: { Authorization: `Bearer ${body.openrouterKey}` } })).ok)
        return json({
  ...
  if (body.snapshotId)
    setSetting("snapshot_id", body.snapshotId);
  if (body.repoUrl?.trim())
    setSetting("repo_url", body.repoUrl.trim());
  if (body.gitToken)
    setSetting("git_token", body.gitToken);
  ...
  if (body.openrouterKey)
    setSetting("openrouter_key", body.openrouterKey);
  if (body.region)
    setSetting("region", body.region);
  if (body.size)
    setSetting("size", body.size);
...
//# debugId=3E4F21160880974464756E2164756E21
